...
class calc:
	def grad(f: torch.Tensor, position: torch.Tensor, edge_order = 1):
		...
		df_dx = torch.gradient(F[:, :, 0], spacing=position[:, :, 0], dim=-1, edge_order=edge_order)
		df_dy = torch.gradient(F[:, :, 0], spacing=position[:, :, 1], dim=-1, edge_order=edge_order)
		df_dz = torch.gradient(F[:, :, 0], spacing=position[:, :, 2], dim=-1, edge_order=edge_order)
	...
	@staticmethod
	def conv_oper(F: torch.Tensor, A: torch.Tensor, position: torch.Tensor, edge_order = 1):
		
		dFx_dx = torch.gradient(F[:, :, 0], spacing=position[:, :, 0], dim=-1, edge_order=edge_order)
		dFx_dy = torch.gradient(F[:, :, 0], spacing=position[:, :, 1], dim=-1, edge_order=edge_order)
		dFx_dz = torch.gradient(F[:, :, 0], spacing=position[:, :, 2], dim=-1, edge_order=edge_order)
		
		dFy_dx = torch.gradient(F[:, :, 1], spacing=position[:, :, 0], dim=-1, edge_order=edge_order)
		dFy_dy = torch.gradient(F[:, :, 1], spacing=position[:, :, 1], dim=-1, edge_order=edge_order)
		dFy_dz = torch.gradient(F[:, :, 1], spacing=position[:, :, 2], dim=-1, edge_order=edge_order)
		
		dFz_dx = torch.gradient(F[:, :, 2], spacing=position[:, :, 0], dim=-1, edge_order=edge_order)
		dFz_dy = torch.gradient(F[:, :, 2], spacing=position[:, :, 1], dim=-1, edge_order=edge_order)
		dFz_dz = torch.gradient(F[:, :, 2], spacing=position[:, :, 2], dim=-1, edge_order=edge_order)
		
		out_x = torch.Tensor([A[:, :, 0] * dFx_dx + A[:, :, 1] *  dFx_dy + A[:, :, 2] *  dFx_dz])
		out_y = torch.Tensor([A[:, :, 0] * dFy_dx + A[:, :, 1] *  dFy_dy + A[:, :, 2] *  dFy_dz])
		out_z = torch.Tensor([A[:, :, 0] * dFz_dx + A[:, :, 1] *  dFz_dy + A[:, :, 2] *  dFz_dz])
...
class IdealMHD(nn.Module):
	...
	def Induction(self, B: torch.Tensor, v: torch.Tensor, r: torch.Tensor, edge_order = 1):
		"""
		B: magnetic field | torch.tensor | (batch_size, sequence_length, 3)
		v: mass velocity field | torch.tensor | (batch_size, sequence_length, 3)
		"""
		return ((calc.dF_dt(B, self.step_size, edge_order) - calc.rot(torch.cross(v, B, dim = -1), r, edge_order))**2).mean()
	...
	def MotionConstraint(self, B, J, r, edge_order = 1):
		...
		B: magnetic field | torch.tensor | (batch_size, sequence_length, 3)
		"""
		return ((torch.cross(J, B, dim = -1) - calc.conv_oper(B, B, r, edge_order)/mu_0 + calc.grad((torch.norm(B, 2, -1, True)**2)/(2*mu_0)))**2).mean()
	...
	def forward(
		...
		return (torch.tensor(lambdas)*torch.tensor([
