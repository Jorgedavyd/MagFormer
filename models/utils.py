...
def SingularLayer(input_size, output, activation):
    out = nn.Sequential(nn.Linear(input_size, output), activation())
    return out
...
class DeepNeuralNetwork(nn.Module):
    def __init__(
        ...
        assert len(hidden_activations) == len(
            architecture
        ), "Must have activation for each layer, if not, put None as activation"
        self.overall_structure = nn.Sequential()
        # Model input and hidden layer
        for num, (output, activation) in enumerate(
            zip(architecture, hidden_activations)
        ):
            self.overall_structure.add_module(
                name=f"layer_{num+1}",
                module=SingularLayer(input_size, output, activation),
            )
            input_size = output
...
class RootMeanSquaredNormalization(nn.Module):
    ...
    def forward(self, x_n):
        ...
        return self.g_i * (
            x_n * torch.rsqrt(x_n.pow(2).mean(-1, keepdim=True) + self.eps)
        )
    ...
    def flashattention(queries, keys, values, mask=None, dropout=0, scale: float = 1):
        with torch.backends.cuda.sdp_kernel(
            ...
            out = F.scaled_dot_product_attention(
                queries, keys, values, mask, dropout, scale=scale
            )
        return out
...
class MonteCarloFC(nn.Module):
    def __init__(self, fc_layer, dropout: float = 0.2, n_sampling: int = 5):
        super().__init__()
...
class DnPositionalEncoding(nn.Module):
    def __init__(self, delta_t: timedelta, degree: int = 1, edge_order: int = 1):
        super().__init__()
...
class ModedTimeSeriesAttention(nn.Module):
    def __init__(
        self, d_model: int, seq_len: int, num_heads: int, kdim: int, vdim: int
    ):
        super().__init__()
        """
        ...
        assert (
            vdim % num_heads == 0
        ), "k = \frac{hiden_dim}{num_heads} | k \in \mathbb(Z)^{+}"
        assert (
            seq_len % num_heads == 0
        ), "k = \frac{hiden_dim}{num_heads} | k \in \mathbb(Z)^{+}"
    ...
    def forward(self, queries, keys, values, mask=None):
        ...
        Q_time = (
            Q.transpose(-1, -2)
            .view(B, self.vdim, self.num_heads, self.time_dim)
            .transpose(1, 2)
        )
        K_time = (
            K.transpose(-1, -2)
        ...
        out_time = (
            Attention.flashattention(Q_time, K_time, V_time, scale=sqrt(self.seq_len))
            .transpose(1, 2)
            .view(B, self.vdim, self.seq_len)
            .transpose(-1, -2)
        )

        # (B,S,num_heads, head_dim) -> (B,S,hidden_dim)
        stan_out = Attention.flashattention(
            Q, K, V, mask, scale=sqrt(self.d_model)
